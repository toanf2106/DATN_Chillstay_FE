  ...
  setup() {
    ...
    const fetchData = async () => {
      try {
        ...
        const filteredForHomestay = reviewsRes.data.filter(r => r.homeStayId == homestayId && r.trangThai !== false);
    ...
    const openAddReviewModal = () => { isReviewModalVisible.value = true; };
    const closeReviewModal = () => { isReviewModalVisible.value = false; };
    const submitReview = () => { /* Logic gửi đánh giá */ };
...
.page-header h1 { font-size: 24px; color: #1a202c; font-weight: 700; }
.back-link { text-decoration: none; color: #4a5568; font-size: 16px; display: flex; align-items: center; gap: 8px; padding: 8px 12px; border-radius: 8px; transition: background-color 0.2s; }
.back-link:hover { background-color: #f7fafc; }
.average-rating-container { display: flex; align-items: center; gap: 10px; margin-bottom: 20px; padding: 15px; background-color: #fef9e7; border-radius: 8px; border: 1px solid #fceecf; }
.average-rating-container .average-score { font-size: 24px; font-weight: bold; color: #d4ac0d; }
.average-rating-container .rating-stars i { color: #f5b041; }
.average-rating-container .total-reviews { font-size: 14px; color: #666; }
.review-filter-controls { display: flex; flex-wrap: wrap; justify-content: flex-start; gap: 10px; align-items: center; margin-bottom: 20px; padding: 10px; background-color: #f9f9f9; border-radius: 8px; }
.review-filter-controls span { font-weight: 500; color: #333; margin-right: 10px; }
.review-filter-controls button { padding: 8px 12px; border: 1px solid #ddd; background-color: #fff; border-radius: 20px; cursor: pointer; transition: all 0.2s ease; }
.review-filter-controls button.active { background-color: #ee4d2d; color: white; border-color: #ee4d2d; }
.review-filter-controls button:hover:not(.active) { background-color: #f0f0f0; }
.loading-state, .empty-state { text-align: center; padding: 60px 0; font-size: 18px; color: #718096; }
.empty-state i { font-size: 48px; margin-bottom: 15px; color: #cbd5e0; }
.reviews-list { display: flex; flex-direction: column; gap: 20px; }
.danh-gia-card { border: 1px solid #e2e8f0; background-color: #fff; padding: 20px; border-radius: 12px; }
.review-header { display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 10px; }
.review-user-info { display: flex; align-items: flex-start; }
.user-avatar { width: 40px; height: 40px; border-radius: 50%; overflow: hidden; margin-right: 12px; flex-shrink: 0; }
.user-avatar img { width: 100%; height: 100%; object-fit: cover; }
.user-info { flex-grow: 1; }
.username-link { text-decoration: none; color: #2d3748; font-weight: 600; font-size: 15px; margin-bottom: 4px; display: inline-block; }
.username-link:hover { text-decoration: underline; }
.rating-stars { display: flex; gap: 2px; }
.rating-stars i { color: #d1d5db; font-size: 14px; }
...
.review-metadata { margin-left: 52px; color: #718096; font-size: 13px; margin-bottom: 12px; }
.review-content { margin-left: 52px; color: #4a5568; font-size: 14px; line-height: 1.6; margin-bottom: 15px; }
.review-image-gallery { display: flex; flex-wrap: wrap; gap: 10px; margin-top: 15px; margin-left: 52px; }
.gallery-item { width: 100px; height: 100px; border-radius: 8px; overflow: hidden; border: 1px solid #e2e8f0; }
.gallery-item img { width: 100%; height: 100%; object-fit: cover; }
.no-images-placeholder { margin-left: 52px; margin-top: 10px; color: #a0aec0; font-size: 13px; font-style: italic; }
.add-review-btn { background-color: #007bff; color: white; border: none; border-radius: 8px; padding: 10px 20px; font-size: 15px; font-weight: 500; cursor: pointer; display: inline-flex; align-items: center; gap: 8px; box-shadow: 0 2px 8px rgba(0, 123, 255, 0.2); transition: background-color 0.3s, transform 0.2s; }
.add-review-btn:hover { background-color: #0056b3; transform: translateY(-2px); }
